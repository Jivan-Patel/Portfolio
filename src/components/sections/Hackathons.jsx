    ...
    return createPortal(
        ...
        >
            <motion.div
                ...
                className="bg-primary border border-slate-700/60 rounded-2xl w-full max-w-5xl max-h-[90vh] overflow-y-auto shadow-2xl relative flex flex-col hide-scrollbar"
                onClick={e => e.stopPropagation()}
            >
                {/* Close Button */}
                <button
                ...
                <div className="bg-secondary/40 p-6 sm:px-8 border-b border-slate-700/50 relative overflow-hidden flex-shrink-0">
                    ...
                    <div className="z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                            <h3 className="text-2xl lg:text-3xl font-bold text-slate-100 flex items-center gap-3">
                                {hackathon.title}
                            </h3>
                            <div className="flex flex-wrap items-center gap-x-6 gap-y-2 mt-3 text-sm text-slate-400 font-medium">
                                {hackathon.date && (
                        ...
                        {hackathon.achievement && (
                            <div className="inline-flex shrink-0 items-center gap-2.5 px-5 py-2.5 bg-gradient-to-r from-amber-500/10 to-amber-600/10 text-amber-400 border border-amber-500/30 rounded-full font-bold shadow-[0_0_15px_rgba(245,158,11,0.15)] mt-2 sm:mt-0 text-sm tracking-wide">
                ...
                <div className="p-6 sm:p-8 grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 flex-shrink-0">
                    ...
                    {hackathon.projectScreenshot && (
                        <div className="flex items-start justify-center">
                            <div className="w-full rounded-xl overflow-hidden shadow-[0_4px_20px_rgba(0,0,0,0.3)] border border-slate-700/60 bg-black aspect-video lg:aspect-[4/3] max-h-[340px]">
                                <img
                    ...
                    <div className="flex flex-col h-full gap-5">
                        <div>
                            ...
                            <p className="text-[16px] text-slate-300 opacity-80 leading-relaxed font-light">
                                {hackathon.projectDescription}
                            </p>
                        </div>

                        {hackathon.keyLearnings && (
                            <div className="mt-2 p-5 rounded-xl bg-gradient-to-br from-slate-800/60 to-slate-900/40 backdrop-blur-sm border border-slate-700/50 flex items-start gap-4">
                                <div className="shrink-0 p-2 bg-accent/10 rounded-lg text-accent text-lg">💡</div>
                                <div className="pt-0.5">
                                    <span className="font-bold text-slate-200 tracking-wide text-[13px] uppercase mb-1 block opacity-90">Core Insight</span>
                                    <span className="leading-relaxed text-sm text-slate-400">{hackathon.keyLearnings}</span>
                                </div>
                            </div>
                        )}
                        ...
                        <div className="flex flex-wrap gap-2.5 mt-2">
                            {hackathon.tech.map((tag) => (
                                <span key={tag} className="text-[13px] font-semibold px-3 py-1.5 bg-secondary/80 text-slate-300 rounded-lg border border-slate-700/50 shadow-sm">
                                    {tag}
                                </span>
                            ))}
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row flex-nowrap gap-3 mt-auto pt-4 w-full">
                            {hackathon.demo && (
                                <a href={hackathon.demo} target="_blank" rel="noopener noreferrer" className="flex-1 flex justify-center items-center gap-2 px-3 py-3 rounded-lg bg-accent text-primary font-bold text-sm lg:text-base hover:bg-accent/90 transition-all shadow-md shadow-accent/20 active:scale-95 whitespace-nowrap">
                                    <FaExternalLinkAlt size={14} /> Live Demo
                                </a>
                            )}
                            {hackathon.github && (
                                <a href={hackathon.github} target="_blank" rel="noopener noreferrer" className="flex-1 flex justify-center items-center gap-2 px-3 py-3 rounded-lg border-2 border-slate-600 font-bold text-slate-300 text-sm lg:text-base hover:text-white hover:border-slate-400 hover:bg-slate-800 transition-all active:scale-95 whitespace-nowrap">
                                    <FaGithub size={16} /> View Code
                                </a>
                            )}
                            {hackathon.certificate && (
                                <a href={hackathon.certificate} target="_blank" rel="noopener noreferrer" className="flex-1 flex justify-center items-center gap-2 px-3 py-3 rounded-lg border-2 border-amber-500/20 bg-amber-500/10 font-bold text-amber-400 text-sm lg:text-base hover:text-amber-300 hover:border-amber-500/50 hover:bg-amber-500/20 transition-all active:scale-95 whitespace-nowrap">
                ...
                {hackathon.eventPhotos && hackathon.eventPhotos.length > 0 && (
                    <div className="border-t border-slate-700/30 p-6 sm:p-8 bg-black/10 flex-shrink-0">
                        <h4 className="text-xs font-bold text-slate-400 uppercase tracking-[0.15em] flex items-center gap-2.5 mb-5 opacity-90">
                        ...
                        <div className="grid gap-4" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))' }}>
                            {hackathon.eventPhotos.map((photo, index) => (
                                <div
                                    ...
                                    className="relative rounded-xl overflow-hidden border border-slate-700/50 aspect-[4/3] cursor-zoom-in group/gallery hover:border-accent/50 transition-all"
                                >
                                    <div className="absolute inset-0 bg-primary/20 group-hover/gallery:bg-transparent transition-colors z-10 duration-300" />
                                    <img src={photo} alt={`Event Photo ${index + 1}`} className="w-full h-full object-cover group-hover/gallery:scale-[1.05] transition-transform duration-500" />
                                </div>
                            ))}
                        </div>
                    </div>
...
const HackathonCard = ({ hackathon, onClick }) => {
    return (
        <motion.div
            ...
            onClick={onClick}
            className="flex flex-col bg-secondary/30 rounded-2xl overflow-hidden border border-slate-700/50 hover:border-accent/40 shadow-lg hover:shadow-[0_8px_25px_rgba(var(--accent-rgb),0.15)] transition-all cursor-pointer group"
        >
            {/* Thumbnail */}
            <div className="relative aspect-video w-full overflow-hidden bg-black border-b border-slate-700/50">
            ...
            <div className="p-5 flex flex-col flex-1 gap-2.5">
                <h4 className="text-[18px] font-bold text-slate-100 group-hover:text-accent transition-colors leading-tight line-clamp-1">
                    {hackathon.title}
                </h4>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                    {hackathon.demo && (
                        <a
                            ...
                            onClick={(e) => e.stopPropagation()}
                            className="inline-flex items-center gap-1.5 self-start text-xs font-semibold text-primary bg-accent hover:bg-accent/90 px-3 py-1.5 rounded-md transition-colors"
                        >
                    ...
                    {hackathon.github && (
                        <a
                            ...
                            rel="noopener noreferrer"
                            onClick={(e) => e.stopPropagation()}
                            className="inline-flex items-center gap-1.5 self-start text-xs font-semibold text-slate-300 border border-slate-600 hover:text-white hover:bg-slate-700 px-3 py-1.5 rounded-md transition-colors"
                        >
                ...
                <div className="mt-auto pt-3 flex items-center text-xs text-slate-500 font-medium tracking-wide w-full justify-between">
                    ...
                    {hackathonData.length > 6 && (
                        hasMore ? (
                            <motion.button
                                ...
                                className="px-10 py-3.5 rounded-full border-2 border-accent text-accent font-bold hover:bg-accent hover:text-primary transition-colors shadow-[0_0_15px_rgba(var(--accent-rgb),0.2)]"
                            >
                                Load More Hackathons
                            </motion.button>
                        ) : (
                            <div className="relative w-full max-w-sm flex items-center justify-center p-4">
                                <div className="absolute w-full h-[1px] bg-gradient-to-r from-transparent via-slate-700/60 to-transparent"></div>
                                <span className="bg-primary px-5 text-slate-500 text-sm font-semibold tracking-wider uppercase z-10 opacity-70">End of records</span>
                            </div>
                        )
                    )}
                </div>
